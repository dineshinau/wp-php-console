...
class Connector {
	...
	const SERVER_COOKIE           = 'php_console_server';
	const CLIENT_INFO_COOKIE      = 'php_console_client';
	...
	public function startEvalRequestsListener( $exitOnEval = true, $flushDebugMessages = true ) {
		...
		if ( $this->isActiveClient() && $this->isAuthorized() && isset( $_POST[ self::POST_VAR_NAME ]['eval'] ) ) {
			$request = $_POST[ self::POST_VAR_NAME ]['eval'];
	 ...
	 * @param array  $args
	 */
	protected function convertWalkRecursiveItemEncoding( &$string, $key = null, array $args = array() ) {
	...
	private function setPostponeHeader() {
		...
		$this->setHeaderData(
			$this->serializeResponse(
				new PostponedResponse(
					array(
						'id' => $postponeResponseId,
					)
				)
			),
			self::POSTPONE_HEADER_NAME,
			true
		);
